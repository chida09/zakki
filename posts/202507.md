...
- 公開するボタンを押した後に、リロードするとまた公開するに戻ってしまう問題
- URLパラメータが付与されない状態管理の場合、useEffectを入れるべきだった

### 問題の原因
`useState` の初期化時に `data.isArchivePublished ?? false` を設定していましたが、`useState`は初期レンダリング時にのみ初期値を設定するため、後から `data` の値が変更されても `isPublished` の状態は更新されませんでした。

### 修正内容
`useEffect` を追加して、`data.isArchivePublished` の値が変更されたときに `isPublished` を更新するようにしました。

```ts
  const [isPublished, setIsPublished] = useState<boolean>(
    data?.isArchivePublished ?? false
  );

  useEffect(() => {
    setIsPublished(data?.isArchivePublished ?? false);
  }, [data?.isArchivePublished]);
```
